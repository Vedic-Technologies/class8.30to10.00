    ...
    <div className="bg-gray-200 text-white w-[400px] h-full p-10 mt-20 ">
      ...
      <ul className="flex flex-col gap-3 mt-5 text-black justify-start items-start ">
        {projectList.map((project, index) => (
         <Link to={`/${project.id}`} className="w-full">  <li key={index} className="flex text-lg items-center gap-3 hover:bg-blue-800 hover:text-white py-1 px-2 w-full rounded-md cursor-pointer">
